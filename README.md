...
NetCurfew cuts your internet connection automatically when your PC is locked at night — and requires a password you *and only you* know to bring it back.

[![Windows](https://img.shields.io/badge/Windows-10%2F11-0078d7?logo=windows11&logoColor=white)](https://github.com/ALeXXBody/NetCurfew/releases)
...
[![Release](https://img.shields.io/github/v/release/ALeXXBody/NetCurfew?color=38bdf8)](https://github.com/ALeXXBody/NetCurfew/releases/latest)
...
NetCurfew lives in the system tray and watches Windows lock/unlock events:
...
- 🔐 **Everything is password-gated** — disabling, enabling, changing settings, and even *exiting NetCurfew* require the password. Wrong attempts are logged.
...
| 🚀 **Silent install** | `NetCurfew-1.0-Setup.exe /S` for unattended deployment |
| 🧹 **Clean uninstall** | Optional full removal of configuration, logs and scheduled task |

## 📸 Screenshots

<div align="center">
  <img src="src/screenshot.png" alt="NetCurfew main window" width="60%"/>
  <img src="src/settings_dlg.png" alt="NetCurfew settings" width="30%"/>
</div>

## 📥 Install

1. Grab **`NetCurfew-1.0-Setup.exe`** from the [**latest release**](https://github.com/ALeXXBody/NetCurfew/releases/latest)
2. Right-click → **Run as administrator** (the installer will request elevation via UAC)
3. Follow the wizard — it will:
   - Install to `C:\Program Files\NetCurfew\`
   - Create Start Menu / Desktop shortcuts
   - Register a scheduled task to auto-start NetCurfew at login
...
> **Command line:** `NetCurfew-1.0-Setup.exe /S` performs a silent install.

## ⚙️ Usage

- Run NetCurfew from the Start Menu or desktop shortcut. It minimizes to the **system tray**.
...
Notes:
- Adapter control needs **administrator rights** — NetCurfew runs elevated via the scheduled task and will offer an elevated relaunch otherwise.
- The installer enables Windows audit policies (`Logon`, `Logoff`, `Special Logon`) so lock/unlock events appear in the Security log — that's how NetCurfew detects locking.

## 🗑️ Uninstall

Uninstall from **Windows Settings → Apps** or the Start Menu shortcut. It stops NetCurfew, removes the task, shortcuts and registry entries, and offers to delete your saved config/logs.
...
```bash
./installer/build.sh
# → src/NetCurfew.exe + NetCurfew-1.0-Setup.exe
```
...
```
installer/
  NetCurfew.nsi      NSIS installer script
  build.sh           Full Linux/CI build (windres → mcs → makensis)
src/
  NetCurfew.cs       Application source (WinForms, single file)
  AssemblyInfo.cs    Assembly metadata
  NetCurfew.manifest Application manifest (asInvoker, dpi-aware)
...
NetCurfew controls network adapters but is **not anti-tamper software** — a determined user with admin rights can restore connectivity. It's designed to add friction, not to be a fortress.
